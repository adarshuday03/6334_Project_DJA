...
Results presented are from Excel and Python analytical solutions. Complete Excel workbook and Python files with detailed formula documentation is included in the submission. 
...
\begin{itemize}
    ...
    \item This symmetric cost structure needs equal weighting of stockout and overage probability
    \item For uniform distribution, this balance occurs at the median, which equals the mean (270)
...
\end{table}
\vspace{-1em}
Normal distribution yields higher profit due to concentrated probability around the mean. Lognormal (right-skewed) shifts $Q^*$ upward to hedge against high-demand tail risk. Triangular (mode-centered) reduces optimal order slightly. Refer Technical Appendix.
%==============================================================================
...
For simulation realism, we use discrete uniform demand $D \in \{120, 121, \ldots, 420\}$ with equal probability (1/301 each). Analytical tasks (1 to 4, 6 to 8) use continuous approximation.
...
Python provides aggregate statistics across 500 trials, and the Excel workbook offers iteration-level visibility.
...
Excel results confirm Python findings: mean profit \$370 to \$380 range, 6 to 7\% loss probability, substantial profit variance (\$190+ std dev).
...
\midrule
1 to 199 & \$3.00 & \$2.00 & \$2.00 & 0.5000 & 270.0 & No & 199 \\
200 to 399 & \$2.85 & \$1.93 & \$2.15 & 0.5276 & 278.3 & Yes & 278 \\
...
\end{table}
\begin{itemize}
    \item \textbf{Tier 1 (\$3.00):} $Q^* = 270$ exceeds tier maximum (199), so evaluate boundary $Q = 199$
    \item \textbf{Tier 2 (\$2.85):} $Q^* = 278$ falls within [200, 399], this is a feasible interior solution
    \item \textbf{Tier 3 (\$2.70):} $Q^* = 286$ below tier minimum (400), so evaluate boundary $Q = 400$
\end{itemize}
\begin{table}[H]
\centering
\small
...
\begin{itemize}
    ...
    \item Ordering 400 units to access \$2.70 pricing forces overage (131 units expected leftover)
